...
func attack(charName, charClass string) string {
	switch charClass {
	case "warrior":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randDamage(3, 5))
	case "mage":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randDamage(5, 10))
	case "healer":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randDamage(-3, -1))
	}
...
func defence(charName, charClass string) string {
	switch charClass {
	case "warrior":
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randDamage(5, 10))
	case "mage":
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randDamage(-2, 2))
	case "healer":
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randDamage(2, 5))
	}
	return "неизвестный класс персонажа"
}
...
func startTraining(charName, charClass string) string {
	...
	for {
		...
		if command != "skip" && command != "attack" && command != "defence" && command != "special" {
			fmt.Println("Неизвестная команда, попробуй заново.")
			continue
		}
		switch command {
		case "skip":
...
func choiceCharClass() string {
	...
	for approveChoice != "y" {
		...
		if charClass != "warrior" && charClass != "mage" && charClass != "healer" {
			fmt.Println("Неизвестный класс персонажа, попробуй заново.")
			continue
		}
